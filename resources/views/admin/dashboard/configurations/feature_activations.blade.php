            ...
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                ...
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    ...
                    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">
                        {{ translate('translation.SettingsList') }}</h1>
                    <!--end::Title-->
                    <!--begin::Breadcrumb-->
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <li class="breadcrumb-item text-muted">
                            <a href="{{ route('dashboard') }}"
                                class="text-muted text-hover-primary">{{ translate('translation.Dashboard') }}</a>
                        </li>
                        <!--end::Item-->
                        <!--begin::Item-->
                        ...
                        <li class="breadcrumb-item text-muted">
                            <a href="{{ route('features.index') }}"
                                class="text-muted text-hover-primary">{{ translate('translation.Settings') }}</a>
                        </li>
                        <!--end::Item-->
                    </ul>
                    <!--end::Breadcrumb-->
                            ...
                            <div class="d-flex align-items-center position-relative my-1">
                                ...
                                <form action="{{ route('features.index') }} " method="GET">
                                    <input type="text" data-kt-resource-table-filter="search"
                                        class="form-control form-control-solid w-250px ps-14" placeholder="بحث عن اعدادات"
                                        name="keyword" />
                                </form>
                            </div>
                            <!--end::Search-->
                            ...
                            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_resources">
                                ...
                                <thead>
                                    ...
                                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-125px">{{translate('translation.name')}}</th>
                                        <th class="min-w-125px">{{translate('translation.status')}}</th>
                                        <th class="text-end min-w-100px px-10">
                                            {{translate('translation.procedures')}}
                                        </th>
                                    </tr>
                                    <!--end::Table row-->
                                </thead>
                                ...
                                <tbody class="text-gray-600 fw-semibold">
                                    @foreach ($featureFlags as $featureFlag)
                                        <tr>
                                            ...
                                            <td data-kt-features-table-filter="enabled_column">
                                                {{ $featureFlag->enabled ? 'Yes' : 'No' }}</td>

                                            <td class="text-end min-w-100px px-10">
                                                @can('update', $featureFlag)
                                                    <form action="{{ route('features.update') }}" method="POST"
                                                        style="display: inline-block;"
                                                        data-kt-features-table-filter="active_form">
                                                        @csrf
                                                        ...
                                                        <div class="form-check form-check-solid form-switch form-check-custom">
                                                            <input class="form-check-input w-40px h-20px" type="checkbox"
                                                                data-kt-features-table-filter="active_row" id="featureToggle"
                                                                name="featureToggle"
            ...
            $(document).ready(function() {
                ...
                activeButtons.forEach(d => {
                    d.addEventListener('change', function(e) {
                        ...
                        const activeForm = parent.querySelector(
                            '[data-kt-features-table-filter="active_form"]');
                        let Url = activeForm.action;
                        let method = activeForm.method;
                        let featureId = activeForm.querySelector('[date-id="id"]').value;
                        let csrfToken = $('meta[name="csrf-token"]').attr('content');
                        let enabledColumn = parent.querySelector(
                            '[data-kt-features-table-filter="enabled_column"]');
                        $.ajax({
                            url: Url,
                            type: method,
                            headers: {
